...
server <- function(input,output){
  ...
  data <- reactive({
    ...
    data <- read.csv(inFile$datapath, header = input$theader)
    data
  })
  
  tdata <- reactive({
    inFile <- input$tchosenfile
    if (is.null(inFile)) return(NULL)
    data <- read.csv(inFile$datapath, header = input$theader)
    data
  })
  ...
  output$smry1 <- renderPrint({
    str(data.frame(data()))
  })

  output$smry2 <- renderPrint({
    str(data.frame(tdata()))
  })
  ...
  data2ck <- eventReactive(input$convertb,{
    ...
    else if (input$toVarOpts == "factor"){
      old_data[,ipvar] = as.factor(old_data[,ipvar])
    }
  ...
  output$smrybv <- renderPrint({
    ...
    if (cnt>0){
      ...
      else if (  is.factor(dset[,bivarx]) == FALSE &  is.factor(dset[,bivary]) == TRUE ) {
        cat("One variable is Categorical & other is Continuous\n")
        cat("Criteria for Significance: Pr(>F) < 0.05 \n")
        cat("\n")
  ...
  STextinputC <- eventReactive(input$actionc, {
    ...
    if (input$selectc =="Logistic_Regression"){
      ...
      print(paste("SVM Linear classifier Results"))
      print(paste("Train accuracy",round(acc*100,2)))
      print(paste("Test accuracy",round(acct*100,2)))
      
      print(paste("Summary of SVM Model"))
      ...
      print(paste("SVM Polynomical Kernel classifier Results"))
      print(paste("Train accuracy",round(acc*100,2)))
      print(paste("Test accuracy",round(acct*100,2)))
      ...
      print(paste("SVM RBF kernel classifier Results"))
      print(paste("Train accuracy",round(acc*100,2)))
      print(paste("Test accuracy",round(acct*100,2)))
